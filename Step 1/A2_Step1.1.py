...
# PLOTS
plt.figure()
plt.plot(list(T), P_DA_one_price, linewidth=2, label="One-Price")
plt.xlabel("Hour", fontsize = 12)
plt.xticks(list(T), rotation=45)
plt.ylabel("Day-ahead offer [MW]", fontsize = 12)
plt.title("Optimal Hourly Day-Ahead Offers" , fontsize = 14)
plt.legend(frameon=True)
plt.grid(True, linestyle='--', alpha=0.5)
plt.savefig("Two_Scheme_Offers.pdf", bbox_inches="tight")
plt.show()


plt.figure()
plt.hist(scenario_profits_one, bins=40, edgecolor= 'black', label="One-Price")
plt.xlabel("Profit [EUR]", fontsize = 12)
plt.ylabel("Number of Scenarios", fontsize = 12)
plt.title("Profit Distribution", fontsize = 14)
plt.legend(frameon=True)
plt.grid(True, linestyle='--', alpha=0.5)
plt.savefig("Two_Scheme_Histogram.pdf", bbox_inches="tight")
plt.show()
